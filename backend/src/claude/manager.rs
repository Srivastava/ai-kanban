...
pub const CONTEXT_WARN_THRESHOLD: i64 = 120_000;

/// Zone 3 lower bound: Claude generates its own handover document, then claude_session_id
/// is cleared so the next session starts fresh using the handover as context instead of
/// inheriting the full 160K+ token history via --resume.
pub const CONTEXT_HANDOVER_THRESHOLD: i64 = 160_000;
                ...
                if let Some(ref ctx_mgr) = context_manager_for_completion {
                        ...
                        .await
                    {
                        ...
                        if peak_input_tokens >= CONTEXT_HANDOVER_THRESHOLD {
                            // ── Zone 3: Claude-generated handover ──────────────────────────────
                            info!(
                                session_id = %session_id_for_completion,
                                task_id = %session.task_id,
                                peak_input_tokens = peak_input_tokens,
                                threshold = CONTEXT_HANDOVER_THRESHOLD,
                                "Zone 3: context at {}% of handover threshold — generating Claude handover",
                                (peak_input_tokens as f64 / CONTEXT_HANDOVER_THRESHOLD as f64 * 100.0).round() as i64
                            );

                            if let Some(ref csid) = session.claude_session_id {
                                let handover = generate_claude_handover(
                                    &claude_bin_for_completion,
                                    &project_path_for_completion,
                                    csid,
                                )
                                .await;
                                ...
                                match handover {
                                    Some(text) => {
                                        let stored = format!(
                                            "*(Handover generated by Claude — session peak: {} tokens)*\n\n{}",
                                            peak_input_tokens,
                                            text.trim()
                                        );
                                        match task_repo_for_completion
                                            .update_compressed_context(&session.task_id, &stored)
                                            .await
                                        {
                                            Ok(()) => {
                                                info!(
                                                    session_id = %session_id_for_completion,
                                                    task_id = %session.task_id,
                                                    "Stored Claude-generated handover as compressed_context"
                                                );
                                                let _ = session_repo_for_completion
                                                    .update(
                                                        &session_id_for_completion,
                                            ...
                                            Err(e) => {
                                                warn!(
                                                    session_id = %session_id_for_completion,
                                                    error = %e,
                                                    "Failed to store Claude handover — next session will use --resume"
                                                );
                                            }
                                        }
                                    }
                                    None => {
                                        warn!(
                                            session_id = %session_id_for_completion,
                                            "Claude handover generation failed — falling back to LiteLLM compression"
                                        );
                                        if let Some(ref ctx_mgr) = context_manager_for_completion {
                                            let _ = ctx_mgr
                                                .compress_context(
                                ...
                                warn!(
                                    session_id = %session_id_for_completion,
                                    "Zone 3 but no claude_session_id — falling back to LiteLLM compression"
                                );
                                if let Some(ref ctx_mgr) = context_manager_for_completion {
                                    let _ = ctx_mgr
                                        .compress_context(
                        ...
                        } else if peak_input_tokens >= CONTEXT_WARN_THRESHOLD {
                            // ── Zone 2: LiteLLM prep compression (keep --resume) ───────────────
                            info!(
                                session_id = %session_id_for_completion,
                                task_id = %session.task_id,
                                peak_input_tokens = peak_input_tokens,
                                pct_of_warn = pct_of_warn,
                                "Zone 2: context at {}% of warn threshold — LiteLLM prep compression (--resume kept)",
                                pct_of_warn
                            );
                            if let Some(flag_repo) = settings_repo_for_completion.as_ref() {
                                if flag_repo
                            ...
                            tracing::debug!(
                                ...
                                peak_input_tokens = peak_input_tokens,
                                pct_of_warn = pct_of_warn,
                                "Zone 1: context below warn threshold — no compression needed"
                            );
                        }
                    }
                }
