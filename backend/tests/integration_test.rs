...
async fn setup_test_db() -> (TaskRepository, LogRepository, SessionRepository, CommentRepository, TokenEventRepository, SessionMetricsRepository) {
    ...
    (
        ...
        SessionMetricsRepository::new(pool),
    )
}

#[tokio::test]
async fn test_create_task() {
    let (repo, _, _, _, _, _) = setup_test_db().await;
...
async fn test_log_repository_multiple_levels() {
    let (_, log_repo, _, _, _, _) = setup_test_db().await;
...
async fn test_log_repository_combined_filters() {
    let (task_repo, log_repo, _, _, _, _) = setup_test_db().await;
...
async fn test_app_state_new() {
    ...
    let state = AppState::new(task_repo.clone(), log_repo.clone(), session_repo.clone(), comment_repo.clone(), token_event_repo.clone(), session_metrics_repo.clone());
...
async fn test_app_state_into_task_api_state() {
    ...
    let state = AppState::new(task_repo, log_repo, session_repo, comment_repo, token_event_repo, session_metrics_repo);
